...
def create_dependency_graph(package_summaries, output_dir):
    ...
    dot.attr(
        "node", fontname="Arial", fontsize="10", width="2", height="0.5", margin="0.2"
    )
    ...
    with dot.subgraph(name="column_headers") as h:
        ...
        h.node(
            "layers_header", "Layers", shape="none", fontsize="16", fontweight="bold"
        )
        h.node(
            "styles_header", "Styles", shape="none", fontsize="16", fontweight="bold"
        )
        ...
        for product_type in ["single", "grouped"]:
            if product_type in products_by_type:
                ...
                with dot.subgraph(name=f"cluster_{package_name}_{product_type}") as s:
                    ...
                    for product_name, product_info in sorted(
                        ...
                        with dot.subgraph(
                            name=f"cluster_{product_name}_resources"
                        ) as p:
                            # Add layer nodes
                            prev_layer_id = None
...
def create_summary(package_summaries, output_dir):
    ...
    summary_lines.append(
        f"  - Products: {sum(len(summary['products']) for summary in package_summaries.values())}"
    )
    ...
    for package_name, summary in package_summaries.items():
        summary_lines.append(
            print_indented(f"📦 Package: {package_name} ({summary['title']})")
        )
    ...
    json_summary = {
        "statistics": {
            ...
            "products": sum(
                len(summary["products"]) for summary in package_summaries.values()
            ),
